...
<svelte:head>
  <MetaTags
    ...
    description="In this post, we'll discuss how to use rel-events to keep your React app's state sane :]"
    keywords="rel-events, react, redux, big projects, architecture, planning, frontend, javascript, frameworks" />
</svelte:head>

<h1><mark>Keeping global state management sane with rel-events</mark></h1>

<PostMeta date="2019-10-19" text={text} />

<figure><img src={cover} alt="Logos from React and Redux" /></figure>
<article bind:this={el}>
  <p>
    Maintaining a big and complex react app is not for amateurs. Whatever you
  ...
  <p>
    ...
    <strong>real</strong>. It took me a long while to properly understand what
    was going on. Then there are actions, action creators, reducers, dispatches
    and middlewares, and sagas, and async dispatches; it's neverending.
  </p>
  <p>
    After those confusing and long first months, things started to get easier,
    but way,
    <strong>way</strong>
  ...
  <p>
    ...
    logic reactively when events pop up.
  </p>
  <p>
...
`)} />
  <p>
    ...
    have something that manages the event flow, an Event Manager:
  </p>
  <Highlight
    language={javascript}
    code={formatCodeString(`
...
`)} />
  <p>
    ...
    about the inital state of the event as well! Ok, ok, let's see:
  </p>
  <Highlight
    language={javascript}
    code={formatCodeString(`
...
`)} />
  <p>
    That's about it, right? But, hey, how do we trigger it? And how do I make
    the Event register which Components are able to trigger it? How does the
    component get the data from it? We're almost there:
  </p>
  <Highlight
    language={javascript}
    code={formatCodeString(`
...
`)} />
  <p>
    ...
    <a href="https://github.com/labcodes/rel-events/tree/master/docs">in our
      docs</a>.
  </p>
  ...
  <ul>
    <li>
      use a basic Event type (for the times you don't need to make HTTP
      requests);
    </li>
    <li>
    ...
    <li>
      have multiple Events registering to the same Component (you'll probably
      have a Component that needs to listen to multiple Events);
    </li>
    <li>
