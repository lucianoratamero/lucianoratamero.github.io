...
<svelte:head>
  <MetaTags
    ...
    description="In this post, we'll discuss how to decouple rendering from domain login using React and Redux :]"
    keywords="react, redux, big projects, architecture, planning, frontend, javascript, frameworks" />
</svelte:head>
...
<figure><img src={cover} alt="Logos of React and Redux" /></figure>
<article bind:this={el}>
  <p>
    Whenever there’s a new React project, most frontend developers will fumble
    ...
    <strong>not always for the good</strong>. The worst part is that every
    ...
    result: react-redux-api-tools.
  </p>
  <h2>Let’s imagine a common scenario: CRUD</h2>
  <p>
    Since javascript is too permissive, there are endless ways to make a CRUD
    ...
    hooks to make requests. Your code may look a bit like this:
  </p>
  <Highlight
    language={javascript}
...
`)} />
  ...
  <p>
    ...
    using setState callback hells);<br />
    ...
    callbacks.
    <strong>It gets messy. FAST.</strong><br />
  </p>
  <p>
    To make it better, most of us prefer to use a library to provide a global
    application state; one that lives outside all components.
  </p>
  <h2>Second step: use Redux</h2>
  <p>
    Redux is one of the most amazing tools for the job. So let’s say you’re
    ...
    code will probably look a bit like this:
  </p>
  <Highlight
    language={javascript}
    code={formatCodeString(`
...
`)} />
  <p>
    ...
    everything
    <strong>even more coupled</strong>
    and, for sure,
    <strong>worse to read and maintain</strong>. The root cause of the issue is
    that the component is still being responsible for
    <strong>everything</strong>: fetching the data and orchestrating redux
  ...
  <ul class="callout">
    ...
    <li>
      Request is fullfilled asynchronously and triggers success or failure
      reducer
    </li>
...
`)} />
  ...
  <p>
    ...
    <strong>redux middlewares are amazing</strong>. Let’s say we want our
  ...
  <ul class="callout">
    ...
    <li>
      Redux's action describes success and failure reducers, along with which
      function will make the request
    </li>
    <li>
      <strong>Middleware calls the request function, saves the promise and
        triggers the isLoading reducer</strong>
    </li>
    <li>Component renders loading spinner</li>
    <li>
      <strong>Middleware awaits the request and triggers the success or failure
        reducer based on the action's specifications</strong>
    </li>
    <li>
      Success or failure reducer executes business logic and persists response's
      data on the redux's store
    </li>
    <li>Component renders sucess or failure based on redux's data</li>
  </ul>
...
`)} />
  ...
  <p>
    Have you enjoyed this middleware API, want to develop your react-redux app
    ...
    tools! Thanks, and see you later!
  </p>
</article>
