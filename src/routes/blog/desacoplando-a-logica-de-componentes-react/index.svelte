...
<svelte:head>
  <MetaTags
    ...
    description="Nesse post, discutiremos como separar as lógicas de renderização e de domínio usando React e Redux :]"
    keywords="react, redux, grandes projetos, arquitetura, planejamento, frontend, javascript, frameworks" />
</svelte:head>

<h1><mark>Desacoplando a lógica de componentes React</mark></h1>

<PostMeta date="2019-07-23" {text} />
<figure>
...
<article bind:this={el}>
  <p>
    Sempre que há um novo projeto baseado em React, grande parte dos devs
    ...
    <strong>nem sempre pro bem</strong>. O pior é que todo dev frontend que eu
    ...
    requisições e processamento de dados. O resultado: react-redux-api-tools.
  </p>
  <h2>Vamos imaginar um cenário comum: CRUD</h2>
  <p>
    Já que o javascript é muito permissivo, há inúmeras maneiras de
    ...
    código provavelmente irá ficar parecido com isso:
  </p>
  <Highlight
    language={javascript}
...
`)} />
  <p>
    Eu acho que não preciso falar muito sobre isso, mas vou falar de qualquer
    forma: essa implementação, mesmo sendo perfeitamente válida, tem alguns
    <strong>grandes</strong>
    problemas.
  </p>
  <p>
    1.A requisição está sendo feita dentro do componente que, em teoria, deveria
    ter apenas um trabalho:
    <strong>renderizar os dados</strong>. Se, por exemplo, você precisasse
    limpar a sessão do usuário ou executar qualquer lógica de negócio, ela
    provavelmente estaria aqui, o que é
    <strong>ainda pior</strong>;<br />
    ...
    tenha feito essa específica requisição um bilhão de vezes;<br />
    ...
    <strong>O código fica complicado. RAPIDINHO.</strong><br />
  </p>
  <p>
    Para melhorar esse código, muitos de nós preferem usar uma biblioteca para
    prover um estado mais global para a aplicação; um estado que vive fora de
    todos os componentes.
  </p>
  <h2>Segundo passo: usando Redux</h2>
  <p>
    Redux é uma das melhores bibliotecas para esse tipo de trabalho. Digamos que
    ...
    domínio, seu componente vai ficar mais ou menos assim:
  </p>
  <Highlight
    language={javascript}
    code={formatCodeString(`
...
`)} />
  <p>
    Isso resolve grande parte dos problemas, mas eu diria que essa solução
  ...
  <ul class="callout">
    <li>Componente monta e levanta sinal de isLoading pro Redux</li>
    <li>Action do Redux começa a requisição e dispara reducer de isLoading</li>
    <li>Componente renderiza isLoading</li>
    <li>
      Requisição termina assincronamente e dispara reducer de sucesso ou falha
    </li>
    <li>Componente renderiza sucesso ou falha</li>
  </ul>
  <p>
    Note que a única forma de completamente remover a lógica de negócio dos
    componentes é acionando os casos de uso de ‘Sucesso’ e ‘Falha’
    <strong>fora do componente</strong>. Componentes devem renderizar coisas,
    não lidar com gerenciamento de estado da
    <strong>aplicação inteira</strong>. Isso significa que o componente apenas
...
`)} />
  <p>
    E isso significa que… as actions devem fazer a requisição dos dados? E
  ...
  <p>
    Para entregarmos o fluxo ideal de dados,
    <strong>middlewares do redux são maravilhosos</strong>. Imaginem que
    ...
    <em>código do meio</em>
    <strong>fizesse a requisição</strong>
    e, dependendo do resultado,
    <strong>decidisse quais reducers disparar</strong>. Assim, deixaríamos o
    ...
    renderizar coisas. Estou falando sobre algo como isso aqui:
  </p>
  <ul class="callout">
    <li>Componente monta e levanta sinal de iniciar a requisição pro Redux</li>
    <li>
      Action do Redux descreve reducers de sucesso e falha e declara função que
      faz a requisição
    </li>
    <li>
      <strong>Middleware chama a função da requisição, guarda a promise e
        dispara reducer de isLoading</strong>
    </li>
    <li>Componente renderiza isLoading</li>
    <li>
      <strong>Middleware escuta a requisição e chama reducer de sucesso ou falha
        baseado na descrição da action</strong>
    </li>
    <li>
      Reducer de sucesso ou falha executa código de lógica de negócio e persiste
      dados na store
    </li>
    <li>Componente renderiza sucesso ou falha</li>
  </ul>
...
`)} />
  <p>
    Ok, vamos com calma. O que você está vendo aí em cima é a nossa convenção
    atual que descreve uma
    <em>action de requisição</em>, que usa o middleware incluído no nosso pacote
    npm
    <a
      href="https://www.npmjs.com/package/react-redux-api-tools"
      target="_blank"
      rel="noopener"><code>react-redux-api-tools</code></a>. Fazer o dispatch
    dessa action
    <strong>configura</strong>
    o middleware para
    <strong>fazer a requisição</strong>
    (ao chamar a
    <code>apiCallFunction</code>) e para
    <strong>usar o reducer correto</strong>
    assim que a requisição estiver concluída. Enquanto isso, nossos componentes
    e reducers se manteriam inalterados:
  </p>
  <Highlight
    language={javascript}
    code={formatCodeString(`
...
`)} />
  <p>
    E
    <em>voilá,</em>
    nosso fluxo está bem mais limpo, simples e desacoplado!
  </p>
  <h2>Se você gostou disso ou isso parece mágico demais…</h2>
  <p>
    Gostou na API desse middleware? Quer desenvolver seus projetos react-redux
    ...
    pacote npm
    <a
      href="https://www.npmjs.com/package/react-redux-api-tools">react-redux-api-tools</a>.
    ...
    simplificar seu código usando nossas ferramentas! Obrigado, e até a próxima!
  </p>
</article>
