...
<svelte:head>
  <MetaTags
    ...
    keywords="#ajudaLuciano, virtualenv, virtualenvwrapper" />
</svelte:head>
...
<article bind:this={el}>
  <p>
    como eu sou (pessimamente) conhecido por customizar (demais) meu computador
    de trabalho com scripts, aliases e o que for, faz um tempo que o pessoal tem
    me cobrado um post sobre como eu uso virtualenv e virtualenvwrapper. aqui
    vão algumas dicas de como eu uso ambos pra facilitar minha vida como
  ...
  <p>
    ...
    <code>lib</code>,
    <code>include</code>
    e afins, e te dá algumas utilidades. ele coloca as virtualenvs dentro da
    pasta da variável de ambiente
    <code>WORKON_HOME</code>. isso é ótimo pra quem gosta de não sujar a pasta
    do projeto com coisas externas e faz com que você não tenha que colocar as
    pastas da virtualenv no gitignore, por exemplo.
  </p>
  <p>
    no entanto, eu
  ...
  <ul>
    ...
    <li>
      e, o melhor, facilito a vida do meu editor de texto, que vai conseguir ler
      o código das bibliotecas usadas.
    </li>
  </ul>
  <p>
    isso faz com que, por exemplo, meu editor possa me recomendar nomes de
    módulos/classes/funções e faça funcionar o lindo "Go To Definition", que
    abre o local do código-fonte de algum módulo/classe/função que está sendo
    usada no seu código. tanto o Atom quanto o Sublime têm plugins ótimos pra
    "Go To Definition", e isso me salvou inúmeras vezes.
  </p>
  <figure>
  ...
  <p>
    ...
    executar
    <code>workon NOMEDOPROJETO</code>, você fica na mesma pasta onde está, mas
    com todos os parâmetros da sua env, como a versão do python, as bibliotecas
    instaladas, as variáveis de ambiente e tudo mais. serve como ponto de
    entrada pra outros hacks que vou destrinchar e aceita
    <em>tab complete</em>.
  </p>
  ...
  <p>
    ...
    <code>cdvirtualenv</code>
    te leva diretamente pra raiz da pasta que contém sua env sem que você
    precise saber qual ela é. se sua pasta de projeto for a mesma, você ganha o
    bônus de ir diretamente pra pasta do projeto sem precisar dar
    <code>cd</code>
  ...
  <p>
    serve pra você replicar uma virtualenv pra outra. pouco usado, mas salvador
    da pátria quando é.
  </p>
  ...
  <p>
    agora que vêm os maiores pulos do gato (pulos dos gatos?): o
    virtualenvwrapper te dá a opção de você escrever scripts bash pra
    antes/depois de você ativar uma virtualenv. basta colocar um arquivo chamado
    <code>postactivate</code>
    e/ou
    <code>postdeactivate</code>
    na pasta
  ...
  <p>
    ...
    executar um
    <code>cdvirtualenv</code>
    assim que sua env for ativada, te levando direto pra pasta projeto. ou
    então, melhor ainda,
    <strong>iniciar dependências externas da sua aplicação/levantar imagens do
      docker</strong>. ou até fazer com que uma vaquinha diga qual a pasta da
    sua virtualenv (<code>cowsay $VIRTUAL_ENV</code>). o mesmo pra quando você
  ...
  <figure>
    ...
    <a href={runapistar}>
      <img
        alt="vídeo do postactivate iniciando uma aplicação"
        src={runapistar} />
    </a>
  </figure>
  <p>
  ...
  <p>
    enfim, essas foram algumas dicas de como eu automatizo alguns processos
    chatos usando virtualenv e virtualenvwrapper. espero que tenha sido útil pra
    vocês, e caso vocês tenham algum hack similar que ajude no desenvolvimento
    em ambientes python, mandem ver nos comentários! eu vou ficando por aqui.
