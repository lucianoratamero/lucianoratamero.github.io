...
<svelte:head>
  <MetaTags
    ...
    description="percebi que as poucas dificuldades que
    ...
    keywords="python, javascript, frontend, dicas, tutorial" />
</svelte:head>

<h1><mark>Python para desenvolvedores JavaScript</mark></h1>
<PostMeta date="2017-06-08" {text} />
...
<article bind:this={el}>
  <p>
    ...
    <a
      ...
      target="_blank"
      rel="noopener">esse post</a>, eu percebi que as poucas dificuldades que
  ...
  <p>
    ...
    interessante a ideia, então aqui vão algumas diferenças pra se ter em mente
    na hora de virar a chave. algumas são de Mario Pabon, o autor do post
    original, e algumas coisas são minhas mesmo. :)
  </p>
  <p>
...
`)} />
  ...
  <p>
    se a linha do
    <code>print(x)</code>
    tivesse mais um ou menos um espaço antes da chamada da função, o
    interpretador do python levantaria uma exceção
    <code>IndentationError</code>, porque a diferença de espaços criaria um
    bloco inválido.
  </p>
...
`)} />
  ...
  <p>
    ...
    permitidas em javascript não são em python. python prefere levantar um erro
    em vez de fazer uma operação entre tipos que pode gerar um dado errado.
  </p>
...
`)} />
  ...
  <p>
    ao contário de javascript, python tem vários tipos diferentes para
    diferentes números.
  </p>
  <ul>
  ...
  <p>
    você pode fazer as mesmas operações em ambas as linguagens, como, por
    exemplo, soma, subtração, divisão, multiplicação e potenciação.
  </p>
  <h3 id="listas">listas</h3>
  <p>
    listas em python são bem parecidas com as de javascript, com alguns adereços
    a mais. elas podem conter uma mistura de tipos:
  </p>
...
`)} />
  ...
  <p>
    strings em python são praticamente iguais às de javascript. você pode
    acessar cada caractere individualmente, mas só pode alterar o conteúdo ao
    reassinalar a variável.
  </p>
...
`)} />
  ...
  <p>
    dicionários são listas associativas em python, muito parecidos com objetos
    em javascript. eles são declarados praticamente da mesma forma.
  </p>
...
`)} />
  ...
  <p>
    ...
    silenciosamente, temos um método padrão dos dicionários para resgatar
    valores default no caso de eles não terem sido declarados:
  </p>
...
`)} />
  ...
  <p>
    assim como em javascript, funções também são objetos em python. isso
    significa que você pode passar funções como argumentos/callbacks ou até
    assinalar propriedades pra funções.
  </p>
...
`)} />
  ...
  <p>
    ao contrário de ES6, em que você tem que declarar o que você tá exportando
    pro seu módulo, em python você sõ precisa criar um arquivo
    <code>.py</code>
    e suas punções/classes/oquefor estão prontas pra serem importadas. basta
    que, na mesma pasta do seu módulo, você coloque um arquivo
...
`)} />
  ...
  <p>
    em python, assim como ES6, há suporte a classes e herança entre objetos.
    isso é diferente da herança baseada em
...
`)} />
  ...
  <p>
    ...
    qualquer dúvida ou acharem que algo poderia ser explicado melhor, só falar
    nos comentários.
  </p>
  <p>
    e lembrem que a melhor forma de aprender uma linguagem é meter a mão na
    massa e fazer um projetinho! se não souberem por onde começar, eu recomendo
    fortemente o
    <a href="https://tutorial.djangogirls.org/pt/">tutorial das pyladies do
      Django Girls</a>, mas se não quiser aprender com desenvolvimento web, tem
    sempre o
