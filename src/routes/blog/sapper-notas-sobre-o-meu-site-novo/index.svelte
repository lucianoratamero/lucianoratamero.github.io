...
<p>
  primeiro de tudo, o sapper é um framework para aplicações svelte, e só isso já
  é motivo mais do que o suficiente pra adotar. não só isso, mas é um produto do
  mesmo time, com os mesmos valores. o sapper é rápido, gera pouquíssimo código
  e, por padrão, só envia pro front o código estritamente necessário, além de
  prover uma estrutura intuitiva para roteamento, baseado na árvore de
  diretórios.
</p>
...
<p>
  essas limitações vieram por conta desse site estar hospedado no github pages,
  mas acho essas necessidades interessantes pra estressar um pouco os limites de
  uma ferramenta. coube como uma luva. :]
</p>
...
<p>
  ...
  migrar os posts do meu antigo site pra cá do que efetivamente implementando o
  site. nunca tive uma barreira que me fez ficar pensando "nossa, fazer essa
  funcionalidade aqui é impossível". pelo contrário, todos os desafios que tive
  foram pouco complicados, e ter um compilador do lado me dizendo o que eu fiz
  de errado é maravilhoso.
</p>

<h3 id="acessibilidade">acessibilidade</h3>

<p>
  quando falo de ter uma aplicação acessível, não tô falando só sobre UI, sobre
  ela ser facilmente legível para leitores de tela ou ser fácil de usar.
...
<p>
  ...
  facilmente com a plataforma javascript - e, por consequência, o sapper também.
  isso torna possível integrações com inúmeras ferramentas, que ficam
...
<p>
  enquanto seria fácil de fazer paginação via js, minha limitação (autoimposta,
  diga-se de passagem) de permitir acesso com js desabilitado me fez gastar um
  bocado de tempo entendendo como o sapper funciona um pouco mais a fundo, pra
  que as páginas se incrementassem automaticamente depois de eu adicionar um
  post.
</p>
...
<p>
  ...
  também prometeram um caminho de migração razoavelmente facilitado pra quem tem
  aplicações sapper, então não vi muito motivo pra adiar meu site novo.
</p>
...
<p>
  enfim, essas foram minhas notas sobre refazer tudo com svelte/sapper. o código
  fonte tá
  <a
