...
<svelte:head>
  <MetaTags
    ...
    description="Nesse post, discutiremos como usar a rel-events para lidar com o estado global de sua app React de uma maneira mais fácil e sã :]"
    keywords="rel-events, react, redux, grandes projetos, arquitetura, planejamento, frontend, javascript, frameworks" />
</svelte:head>

<h1><mark>Mantendo o estado global de uma maneira sã com rel-events</mark></h1>

<PostMeta date="2019-09-19" {text} />
...
<article bind:this={el}>
  ...
  <p>
    Sempre que eu penso em criar uma biblioteca nova, essa tirinha do xkcd vem
    ...
    qualquer pra separar lógica de negócio/domínio dos meus componentes react.
  </p>
  <p>
    Mas no fim do dia, depois de pensar muito, acabo me rendendo e rodando o
    clássico
    <code>npm install redux react-redux --save</code>. Eu até compartilhei
  ...
  <p>
    Não ache que eu odeio o redux; pelo contrário, eu amo. Ele cumpre sua
    ...
    pela primeira vez é
    <strong>real</strong>. Demorei um bom tempo pra entender o que tava
  ...
  <p>
    Depois dos primeiros confusos meses, lidar com o redux fica mais fácil (como
  ...
  <p>
    Cheguei à conclusão de que seria ótimo ter todas as features que gosto na
  ...
  <p>
    ...
    forma reativa quando eventos acontecem.
  </p>
  <p>
    Pra isso, vou propor que a gente imagine um cenário bem comum: precisamos
    fazer com que um usuário faça login na nossa aplicação.
  </p>
  <p>
    Nós poderíamos fazer todo o fluxo de requisições dentro dos próprios
    componentes, mas
    <a
      href="https://labcodes.com.br/blog/pt/mantendo-o-estado-global-de-uma-maneira-sa-com-rel-events.html">como
      dissemos no nosso outro post</a>, nós não exatamente recomendamos esse
    ...
    <code>mapStateToProps</code>, e às vezes nós não temos tempo de falar sobre
    absolutamente tudo de redux antes de entregar nossos projetos. Nós
    poderíamos usar sagas, mas aí o redux volta pra nos atazanar em dobro.
  </p>
  <p>
    Então, vamos por uns minutos esquecer de actions e reducers e imaginemos um
...
`)} />
  <p>
    Legal, mas onde colocaríamos todo seu comportamento? Onde tá o código que
...
`)} />
  <p>
    ...
    Como que o Componente pegaria os dados desse evento? Tamo quase lá:
  </p>
  <Highlight
    language={javascript}
    code={formatCodeString(`
...
`)} />
  <p>
    ...
    <a href="https://github.com/labcodes/rel-events/tree/master/docs">na nossa
      documentação (ainda só em inglês, com português em breve</a>).
  </p>
  ...
  <ul>
    <li>
      usar um Evento (Event) básico (pras horas em que você não precisa fazer
      requisições);
    </li>
    <li>
      encadear um Event/HTTPEvent em outro (útil pra quando você quer buscar
      mais dados depois que um evento teve sucesso);
    </li>
    <li>
      ter múltiplos Events registrados para um mesmo Componente (porque você
      provavelmente vai ter um Componente que precisa ler dados de vários
      lugares);
    </li>
    <li>
      executar código depois das mudanças de estado de um Evento que foi
      disparado (usando
      <code>afterDispatch</code>);
    </li>
    <li>
      executar código depois das mudanças de estado de uma requisição que teve
      sucesso (usando
      <code>afterSuccess</code>);
    </li>
    <li>
      executar código depois das mudanças de estado de uma requisição que falhou
      (usando
      <code>afterFailure</code>);
    </li>
    <li>
      avaliar se um evento deve ser disparado ou não (com o método
      <code>shouldDispatch</code>
      do EventManager).
    </li>
  </ul>
  <p>
    E muito mais! Tudo isso sem a carga cognitiva, sem as constantes aleatórias,
    ...
    dia-a-dia. Mesmo. Eu sei, porque tô usando!
  </p>
  <p>
    Então se você gostou,
    <a href="https://github.com/labcodes/rel-events">dá uma olhada no nosso
